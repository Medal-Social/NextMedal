...
for (let attempt = 1; attempt <= 6; attempt++) {
  try {
    for (const path of ['/', '/robots.txt']) {
      ...
      const response = await fetch(url, { signal: AbortSignal.timeout(20000), headers: { 'User-Agent': 'Medal-Deployment-Smoke/1.0' } });
      if (!response.ok) throw new Error(`${path}: HTTP ${response.status}`);
      const text = await response.text();
      if (path === '/' && (!response.headers.get('content-type')?.includes('text/html') || !/<html[\s>]/i.test(text))) throw new Error('Homepage did not return HTML');
      if (path === '/robots.txt' && !/user-agent:/i.test(text)) throw new Error('robots.txt did not return robots directives');
    ...
    for (const [path, expectedStatus, contentType] of checks) {
      ...
      if (response.status !== expectedStatus) throw new Error(`${path}: HTTP ${response.status}, expected ${expectedStatus}`);
      if (contentType && !response.headers.get('content-type')?.startsWith(contentType)) throw new Error(`${path}: expected image content`);
