...
import { usePageState, usePagination } from '@/lib/usePagination';
...
describe('usePagination', () => {
  ...
  describe('basic pagination', () => {
    ...
    it('uses default itemsPerPage of 3', () => {
...
describe('Pagination component', () => {
  ...
  it('returns null when atStart and atEnd are both true (single page)', () => {
    const items = [1, 2];
    const { result } = renderHook(() => usePagination({ items, itemsPerPage: 3 }));

    const PaginationComponent = result.current.Pagination;
    const rendered = PaginationComponent({});

    expect(rendered).toBeNull();
  });

  it('returns null when no paginated items', () => {
    const { result } = renderHook(() => usePagination({ items: [], itemsPerPage: 3 }));
