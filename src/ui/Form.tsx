...
export default function Form({ form, className }: FormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await submitForm({
      ...
      data: {
        ...formData,
        _submissionTimestamp: mountTime,
      },
    });
  ...
  const acceptanceUrl = form.acceptance?.link
    ? resolveSlug({
        _type: form.acceptance.link.internal?._type,
        internal: form.acceptance.link.internal?.metadata?.slug?.current,
        external: form.acceptance.link.external,
        params: form.acceptance.link.params,
      })
    : undefined;
              ...
              {field.type === 'textarea' ? (
                ...
                <div className="flex items-center space-x-3 py-2 px-1">
                  <input
                    ...
                    checked={!!formData[field.name.current]}
            ...
            >
              ...
              {acceptanceUrl && (
                <a
                  ...
                  rel={
                    form.acceptance.link?.type === 'external' ? 'noopener noreferrer' : undefined
                  }
                  className="text-primary hover:underline ml-1 font-semibold"
                  onClick={(e) => {
                    if (form.acceptance?.link?.type === 'internal') {
                      e.preventDefault();
