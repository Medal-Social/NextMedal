...
>(({ className, children, ...props }, ref) => {
  ...
  const hasDialogTitle = React.Children.toArray(children).some((child) => {
    if (React.isValidElement(child)) {
      // Check if the direct child is a DialogTitle
      if (child.type === DialogTitle) {
        return true;
      }

      // Check nested children (for example, inside DialogHeader)
      if (child.props && typeof child.props === 'object' && 'children' in child.props) {
        const nestedChildren = React.Children.toArray(child.props.children as React.ReactNode);
